...
export class PaddlePortal {
	...
	/** @returns the portal URL (overview, or deep-linked to the subscription if given). */
	async sessionUrl(customerId: string, subscriptionIds: string[]): Promise<string> {
		...
		return s.urls.subscriptions?.[0]?.cancelSubscription ? s.urls.general.overview : s.urls.general.overview;
	}
}
